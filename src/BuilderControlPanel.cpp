    ...
    : QWidget(parent)
{
    ...
    m_layout->addWidget(scrollBuilderArea, 1, 0, 3, 1);

    m_layout->addWidget(getElementsCreatorWidget(this), 0, 1, 2, 1);
    m_layout->addWidget(getResultedTextViewWidget(this, builderContainer), 2, 1, 2, 1);

    ParameterUIBuilder* parameterHolderWidget = new ParameterUIBuilder(builderContainer, this);

    m_layout->addWidget(parameterHolderWidget, 0, 2, 2, 1);

     // Set the column stretch to distribute remaining space
    m_layout->setRowStretch(1, 1);
    m_layout->setRowStretch(2, 2);
